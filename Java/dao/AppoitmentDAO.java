		...
		try {
			...
			int selector=0;
				ResultSet resultSet = statement
						.executeQuery("SELECT * FROM `physicianappointments` WHERE (`APPOINTMENTDATE` BETWEEN '"+new  java.sql.Date(startDate.getTime())+"'  AND '"+new  java.sql.Date(endDate.getTime())+"') AND STATUS = 'ACTIVE' ");
